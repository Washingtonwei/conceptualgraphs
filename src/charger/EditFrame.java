...
//        renameGraphInFrame( graphAbsoluteFile.getPath() ); //initEditGraph( filename );
    ...
    public void setupComponents() throws Exception {
        ...
        messageBox.setBorder( BorderFactory.createTitledBorder( Global.BeveledBorder,
                Global.EditorNameString + " " + Global.CharGerVersion, TitledBorder.RIGHT, TitledBorder.TOP,
...
//            threadgroup = null;
    ...
    public void thisWindowActivated() {
        ...
        Global.refreshWindowMenuList( windowMenu, this );	// "this" here seems important..
...
//        Global.info( "key pressed " + e );
    ...
    public void mousePressed( MouseEvent me ) {
        ...
        if ( !cp.contains( (int)ThisPt.x, (int)ThisPt.y ) ) {
            cp.closeTextEditor();		// just in case
            displayOneLiner( "Are you clicking outside the canvas?" );
            //repaint();
            return;
        }
        
        if ( cp.nodeEditingDialog != null ) cp.closeTextEditor();
        ...
        currentObject = (GraphObject)onTopOfGNode( ThisPt );

        if ( currentObject == null ) {
            currentObject = (GraphObject)insideGEdge( ThisPt );
        }
    ...
    public void mouseReleased( MouseEvent me ) {
        ...
        if ( currentObject != null ) {
            ...
            if ( currentNode != null ) {
                ObjectHistoryEvent he = new ObjectHistoryEvent( "user" );
                he.setDescription( System.getProperty( "user.name" ) );
                currentNode.addHistoryEvent( he );
            }
        }
    ...
    public void mouseDragged( MouseEvent me ) {
        ...
        if ( !alreadyDragging ) {
            ...
            go = (GraphObject)onTopOfGNode( dragStartPt );
    ...
    public void stopDragging( Point2D.Double dragStartPt, Point2D.Double dragStopPt, boolean shiftDown ) {
        ...
        if ( editingToolbar.isEdgeInsertMode() ) {
            go1 = (GraphObject)onTopOfGNode( dragStartPt );
            go2 = (GraphObject)onTopOfGNode( dragStopPt );
            if ( go1 == go2 ) {
                return;
            }
    ...
    public GraphObject onTopOfObject( Point2D.Double p ) {
        Iterator iter = new DeepIterator( TheGraph );
        GraphObject go;
...
//                CGUtil.grow( drect, (  * Graph.contextBorderWidth ), ( 0.5f * Graph.contextBorderWidth ) );
            ...
            } else if ( go.displayRect.contains( p ) ) {
    ...
    public void includeContextMembers() {
        /*Iterator iter = EFSelectedObjects.iterator();
         while ( iter.hasNext() )
         {
         GraphObject go = (GraphObject) iter.next();
         if ( go instanceof Graph ) includeContextMembers( (Graph) go );
         }
         */
        for ( int k = 0; k < EFSelectedObjects.size(); k++ ) {
            GraphObject go = (GraphObject)EFSelectedObjects.get( k );
            if ( go instanceof Graph ) {
    ...
    public void includeContextMembers( Graph g ) {
        Iterator iter = new ShallowIterator( g );
        while ( iter.hasNext() ) {
            GraphObject go = (GraphObject)iter.next();
    ...
    protected void setSelectedGraphObjects( Graph g, Rectangle2D.Double r, boolean select ) {
        ...
        while ( iter.hasNext() ) {  // does not check for object type here!
            go = (GraphObject)iter.next();
            if ( go.displayRect.createUnion( r ).equals( r ) ) // if go is inside selection rectangle
            {
    ...
    public void resetSelection() {
        ...
        if ( somethingHasBeenSelected ) {
            ...
            while ( EFSelectedObjects.size() > 0 ) {
                GraphObject go = (GraphObject)EFSelectedObjects.get( 0 );
                go.isSelected = false;
                EFSelectedObjects.remove( go );
            }

            while ( EFSelectedNodes.size() > 0 ) {
                GraphObject go = (GraphObject)EFSelectedNodes.get( 0 );
                go.isSelected = false;
                EFSelectedNodes.remove( go );
            }
...
//        }
        ...
        // end loop
        Iterator nodes = selectedNodes.iterator();
        while ( nodes.hasNext() ) {
            nodeN = (GraphObject)nodes.next();
        ...
        if ( dominantContext != null ) {
            ...
            contextG.displayRect =  Util.make2DDouble( border);
...
//            contextG.resizeForContents( contextG.getCenter() );
    ...
    public boolean entireGraphSelected( Graph g ) {
        Iterator iter = new ShallowIterator( g, GraphObject.Kind.GNODE );
        if ( !g.isSelected ) {
            return false;
        }
        while ( iter.hasNext() ) {
            if ( !( (GraphObject)iter.next() ).isSelected ) {
                return false;
            }
        }
    ...
    public void clearStatus() {
        ...
        Iterator iter = new DeepIterator( TheGraph, GraphObject.Kind.GNODE );
        while ( iter.hasNext() ) {
...
//        }
    ...
    public void renameGraphInFrame( String filename ) {
        ...
        if ( Global.acceptCGXFileName( filename ) || Global.acceptCGIFFileName( filename ) ) {
            withoutExtension = Util.stripFileExtension( filename );
        }
...
//        } else { */
            ...
            graphName = Util.stripFileExtension( graphSourceFile.getName() );
//        }
