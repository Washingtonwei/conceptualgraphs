...
import charger.util.GenericTextFrame;
import charger.util.Tag;
import charger.util.Util;
import charger.xml.CGXGenerator;
...
//    charger.xml.CGXParser parser = new charger.xml.CGXParser();
    ...
    private Iterator findIterator = null;
...
//        urMgr = new EditStateMgr( ef, maxUndo );
        ...
        // Set up "purpose"  menu
//        GraphModality dummy;
//        Iterator purposes = GraphModality.allLabels();
//        JCheckBoxMenuItem cbmi = null;
//        while ( purposes.hasNext() ) {
//            cbmi = new JCheckBoxMenuItem( (String)purposes.next() );
//            ef.GraphModalityMenu.add( cbmi );
...
//        }
        ...
        ef.ZoomInItem.addActionListener( new ActionListener() {
        ...
        ef.ZoomOutItem.addActionListener( new ActionListener() {
            public void actionPerformed( ActionEvent e ) {
        ...
        ef.ActualSizeItem.addActionListener( new ActionListener() {
            public void actionPerformed( ActionEvent e ) {
        ...
        ef.ShowHistoryMenuItem.addActionListener( new ActionListener() {
            public void actionPerformed( ActionEvent e ) {
                GraphObject go = (GraphObject)ef.EFSelectedObjects.get( 0 );
...
//        }
    ...
    public void performActionClipboardCopy() {
        ...
        /*
        // Handle copying to the system clipboard, using image format from Preferences
...
//        timage.setImage( bi ); // 11-22-2005 : hsd
                ...
                IOManager.ImageTransferable timage = new IOManager.ImageTransferable( bi );
...
//
//            Point2D.Double offset = new Point2D.Double( EditFrame.xoffsetForCopy, EditFrame.yoffsetForCopy );
//            if ( Global.keepClipboardIDs ) {
//                offset = new Point2D.Double( 0, 0 );		// don't offset unless it's a duplicate
//
//            }
//            charger.xml.CGXParser parser = new charger.xml.CGXParser();
//            newOnes = parser.parseCGXMLString( clipcontents, ef.TheGraph, offset, Global.keepClipboardIDs );
        }
        ...
        Iterator iter = newOnes.iterator();
        while ( iter.hasNext() ) {
...
//        ef.resetSelection();
    ...
    public void performActionSelectAll() {
        if ( ( ( ef.cp.nodeEditingDialog != null ) && ef.cp.nodeEditingDialog.isVisible() )
            ...
            Iterator iter = new DeepIterator( ef.TheGraph );
            while ( iter.hasNext() ) {
                GraphObject go = (GraphObject)iter.next();
                ef.addToSelection( go );
            }
    ...
    public void performActionDisplayAsXML() {
        ...
        ef.XMLDisplay.setTheText( CGXGenerator.generateXML( ef.TheGraph ) );
    ...
    public void performActionMakeContext( String what ) {
        if ( ef.somethingHasBeenSelected ) {
            ...
            try {
                Rectangle2D.Double r = Util.make2DDouble( ef.selectionRect );
    ...
    public void performActionColorSelection( String foreback, Color c, Hashtable<String, Color> colorlist ) {
        ...
        if ( ef.somethingHasBeenSelected ) {
            ...
            Iterator iter = ef.EFSelectedObjects.iterator();
            while ( iter.hasNext() ) {
                GraphObject go = (GraphObject)iter.next();
                if ( go.isSelected ) {
    ...
    public void performActionChangeFont() {
        ...
        if ( ef.somethingHasBeenSelected ) {
            Font newFont = ( (GraphObject)ef.EFSelectedObjects.get( 0 ) ).getLabelFont();       // use the first object to initially set the font
...
//            newOnes = parser.parseCGXMLString( s, ef.TheGraph, offset, false );
        }
...
//                IOManager.saveGraph38XML( out, g );
    ...
    public EditorState currentState() {
        EditorState state = new EditorState( CGXGenerator.generateXML( ef.TheGraph ) );
...
//            ArrayList newOnes = parser.parseCGXMLString( state.getGraph(), ef.TheGraph, new Point2D.Double( 0, 0 ), true );
        }
...
//        }
        ...
        layout.setMaxBounds( ef.cp.getBounds());
    ...
    public void performActionReadFromJarfile() {
        Util.getFileFromClassPath( "Graphs", "Samples");
    }
...
//    }
    ...
    public void makeHoldGraph() {
        ...
        OperManager.performActionValidate( ef.TheGraph );
        ef.sp.repaint();
        holdGraph = CGXGenerator.generateXML( ef.TheGraph );
...
//            for ( int k = 0; k < size; k++ ) {
    ...
    public boolean enlargeObject( GraphObject go, int increment ) {
        ...
        if ( go instanceof Graph ) {
            ...
            Iterator iter = new ShallowIterator( (Graph)go );
            while ( iter.hasNext() ) {
                GraphObject ggo = (GraphObject)iter.next();
...
//            g.setFont(  go.getLabelFont());
    ...
    public synchronized static Graph innermostContext( Graph outermost, Point2D.Double p ) {
        ...
        while ( graphs.hasNext() ) {
            g = (Graph)graphs.next();
            if ( g.displayRect.contains( (int)p.x, (int)p.y ) ) {
    ...
    public void performFindAgain( String s ) {
        ...
        while ( findIterator.hasNext() && !found ) {
            GraphObject go = (GraphObject)findIterator.next();
