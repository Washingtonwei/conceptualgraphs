...
/**
 * Frame for choosing UML diagram to convert to CGs.
 *
 * @author Bingyang Wei work based on the class HubFrame by Harry S. Delugach
 *         Copyright (c) 1998-2014
 * 
 */
public class UMLDiagramSelectionFrame extends JFrame implements ManagedWindow {
	...
	public ArrayList<Document> documents = new ArrayList<Document>();// list of document extracted from xmi
	...
	CGButton OpenButton1 = new CGButton();
	CGButton OpenButton2 = new CGButton();
	CGButton OpenButton3 = new CGButton();
	JButton umlButton = new JButton();
...
//	CGButton QuitButton = new CGButton();
	...
	public UMLDiagramSelectionFrame(EditFrame ef) {
		currentEF = ef;
	...
	public void initComponents() // throws Exception
	{
		...
		Instructions.setText(
				"Please select a UML diagram you want to convert to the current CG, and then press the corresponding button...");
		Instructions.setEditable(false);
		Instructions.setFont(new Font("Arial", Font.BOLD + Font.ITALIC, 20));
		Instructions.setLineWrap(true);
		Instructions.setWrapStyleWord(true);

		umlButton.setIcon(new ImageIcon("./src/GIF/UML_logo.gif"));
		umlButton.setLocation(10, 260);
		umlButton.setSize(300, 200);

		GraphFolderLabel.setText("UML diagrams folder:");
		GraphFolderLabel.setForeground(Color.black);
		...
		AvailLabel.setText("UML diagrams available");
		AvailLabel.setForeground(Color.black);
...
//		});
		...
		OpenButton1.setLocation(new Point(670, 90));
		OpenButton1.setSize(300,50);
		OpenButton1.setText("Convert Selected class diagram");
		OpenButton1.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(ActionEvent e) {
				ConvertSelectedClassDiagramsActionPerformed(e);
			}
		});

		OpenButton2.setLocation(new Point(670, 150));
		OpenButton2.setSize(300,50);
		OpenButton2.setText("Convert Selected sequence diagram");
		OpenButton2.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(ActionEvent e) {
				ConvertSelectedSequenceDiagramsActionPerformed(e);
			}
		});

		OpenButton3.setLocation(new Point(670, 210));
		OpenButton3.setSize(300,50);
		OpenButton3.setText("Convert Selected state diagram");
		OpenButton3.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(ActionEvent e) {
				ConvertSelectedStateDiagramsActionPerformed(e);
			}
		});

		OpenAllButton.setLocation(new Point(670, 270));
		OpenAllButton.setSize(300,20);
		OpenAllButton.setText("Convert All UML diagrams");
		OpenAllButton.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(ActionEvent e) {
				ConvertAllUMLDiagramsPerformed(e);
			}
		});

		CloseAllButton.setLocation(new Point(670, 300));
...
//		});
		...
		getContentPane().add(OpenButton1);
		getContentPane().add(OpenButton2);
		getContentPane().add(OpenButton3);
		getContentPane().add(umlButton);
	...
	public void thisWindowClosing(java.awt.event.WindowEvent e) {
		Global.UMLDiagramSelectionFrame = null;
			...
			if (GraphFolderLastModified != ff.lastModified()) {
				...
				String gg[] = ff.list(new FilenameFilter() {
					public boolean accept(File f, String name) {
						if (!name.startsWith(".")) {
							return Global.acceptXMIFileName(name);
						} else {
							return false;
						}
					}
	...
	/**
	 * Get all the selected UML class diagram names, and convert them from objects to
	 * strings, convert them to CGs
	 *
	 * @see UMLDiagramSelectionFrame#openNamedFiles
	 */
	public void ConvertSelectedClassDiagramsActionPerformed(ActionEvent e) {
		...
		currentEF.omgr.performConvertClassDiagramAction(currentEF.TheGraph, documents);
	}
	/**
	 * Get all the selected UML class diagram names, and convert them from objects to
	 * strings, convert them to CGs
	 ...
	 * @see UMLDiagramSelectionFrame#openNamedFiles
	 */
	public void ConvertSelectedSequenceDiagramsActionPerformed(ActionEvent e) {
		Object[] fileNameObjects = GraphDisplayList.getSelectedValuesList().toArray();
		String[] fileNames = new String[fileNameObjects.length];
		for (int entrynum = 0; entrynum < fileNames.length; entrynum++) {
			fileNames[entrynum] = (String) fileNameObjects[entrynum];
		...
		openNamedFiles(fileNames);
		// apply rules to the current graph
		currentEF.omgr.performConvertSequenceDiagramAction(currentEF.TheGraph, documents);
	}
	/**
	 * Get all the selected UML class diagram names, and convert them from objects to
	 * strings, convert them to CGs
	 ...
	 * @see UMLDiagramSelectionFrame#openNamedFiles
	 */
	public void ConvertSelectedStateDiagramsActionPerformed(ActionEvent e) {
		Object[] fileNameObjects = GraphDisplayList.getSelectedValuesList().toArray();
		String[] fileNames = new String[fileNameObjects.length];
		for (int entrynum = 0; entrynum < fileNames.length; entrynum++) {
			fileNames[entrynum] = (String) fileNameObjects[entrynum];
		...
		openNamedFiles(fileNames);
		// apply rules to the current graph
		currentEF.omgr.performConvertStateDiagramAction(currentEF.TheGraph, documents);
	}
...
//				EditFrame newEF = Global.openRuleInNewFrame(files[n]);
				Document doc = Global.getDocumentFromXMI(files[n]);
				numFilesOpened++;
				documents.add(doc);
			} else {
...
//			ConvertSelectedUMLDiagramsActionPerformed(new ActionEvent(e.getSource(), ActionEvent.ACTION_PERFORMED, null));
//		}
	}
	...
	public void ConvertAllUMLDiagramsPerformed(ActionEvent e) {
